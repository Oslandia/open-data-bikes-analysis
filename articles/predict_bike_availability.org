...
is interesting to understand the city organization, the results were especially
impressive for Lyon data set.
...
We can preprocess a little bit more these data so as to extract timestamp
...
this framework, /i.e./ weather forecasts (however we let it for a further study).
...
typical problem for a user who wants to plan a local trip: will he find some
bike at his prefered station, or should he walk to the next one? Or maybe
should he look for an alternative transportation mode, as there is no bike in
the near neighborhood?

Let use two weeks of data for training (/i.e./ from 11/07/11 to 25/07/11) so as
...
Our model learns quite fastly, and after 25 iterations, the training process
...
In the last article, we have plotted the shared bike stations according to
their cluster. Here we are in a regression problem, we are more focused on the
level of bike availability, seen as a percentage.

The following map shows such a quantity, for all Lyon stations:
...
regarding people ability to climb up until these stations...
